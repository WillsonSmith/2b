...
| `PlanPlugin` | Structured multi-step planning with SQLite persistence; injects the active plan as a markdown table every turn via `getContext()`, surviving `MemoryPlugin` summarization; each step can store `notes` (human-readable) and `data` (JSON string) for structured output that downstream steps consume; tools: `create_plan` (goal + steps array), `update_step` (status, optional notes and data), `complete_plan`, `abandon_plan`, `get_plan` (includes step data payloads); public API: `getActivePlan()`, `getPlanById(id)`, `listRecentPlans(limit?)` (omits steps, for UI); pass `dbPath` to constructor (defaults to global `~/.local/share/2b/data/plans.sqlite`; Episteme uses per-workspace `planDbPath(workspaceRoot)`) |
