...
type WsMessage =
  | { type: "snapshot"; messages: ChatMessage[]; state: AgentState; activeTools: ActiveTool[]; dynamicAgents: DynamicAgentRecord[] }
  ...
  | { type: "permission_request"; request: { agentName: string; toolName: string; args: Record<string, unknown> } }
  ...
  return (
    <div className="thinking">
      <div className="thinking-header" onClick={() => setExpanded((x) => !x)}>
        <span className={`thinking-chevron ${expanded ? "thinking-chevron--open" : ""}`}>▶</span>
        <span>Thinking</span>
        {!expanded && <span style={{ color: "var(--text-dim)" }}>({lineCount} {lineCount === 1 ? "line" : "lines"})</span>}
      </div>
  ...
  return (
    <div className="status">
      <div className="status-line">
        ...
        <span className={`status-indicator status-indicator--${isThinking ? "thinking" : "ready"}`}>
          {isThinking ? (
            <>
              <span className="spinner">⟳</span>
              {activeTools.length === 0 ? "thinking" : `${activeTools.length} tool${activeTools.length > 1 ? "s" : ""} running`}
            </>
          ) : (
            "ready"
          )}
        </span>
        {model && <span className="header-model" style={{ marginLeft: "auto" }}>{model}</span>}
      </div>
      ...
      {activeAgents.length > 0 && (
        <div className="status-agents">
          {activeAgents.map((a) => (
            <div key={a.name} className={`status-agent ${a.state === "error" ? "status-agent--error" : ""}`}>
              [{a.name}] {a.state}
            </div>
          ))}
        </div>
    ...
    <div className="permission-overlay">
      <div className="permission-dialog">
        ...
        <div className="permission-buttons">
          <button className="perm-btn perm-btn--yes" onClick={() => onRespond("yes")}>Yes once</button>
          <button className="perm-btn perm-btn--always" onClick={() => onRespond("always")}>Always (session)</button>
          <button className="perm-btn perm-btn--no" onClick={() => onRespond("no")}>No</button>
        </div>
      </div>
    </div>
  );
...
function App() {
  ...
  const handleSlash = useCallback((input: string): boolean => {
    ...
    switch (command) {
      ...
      case "retry": {
        ...
        if (!lastUser) { addSys("No previous message to retry."); return true; }
      ...
      case "copy": {
        ...
        if (!lastAsst) { addSys("No response to copy."); return true; }
  ...
  const handlePermission = useCallback((response: "yes" | "always" | "no") => {
  ...
  return (
    <div className="app">
      <header className="header">
        ...
        <span className="header-model">{connected ? (currentModel || "connected") : "connecting…"}</span>
      </header>
